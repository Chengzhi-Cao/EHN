...
# 第一范数
class get_L1_loss(nn.Module):
    def __init__(self):
        super(get_L1_loss, self).__init__()
    def forward(self, pred, target, trans_feat):
...
# 第二范数
class get_MSE_loss(nn.Module):
    def __init__(self):
        super(get_MSE_loss, self).__init__()
    def forward(self, pred, target, trans_feat):
...
# BCE_Loss
class get_BCE_loss(nn.Module):
    def __init__(self):
        super(get_BCE_loss, self).__init__()
    def forward(self, pred, target, trans_feat):
...
###############################################################################
###############################################################################
class LQGTDataset_mat_SR(data.Dataset):
    """
    ...
    def __init__(self, opt):
        super(LQGTDataset_mat_SR, self).__init__()
        self.opt = opt
        ...
        self.paths_event = get_mat_paths(self.data_type, opt['dataroot_event'])
    ...
    def __getitem__(self, index):
        ...
        event_output = np.array(event_h5f[str(index)])     #[2,260,346]
        event_output = event_output.transpose((1,2,0))

        # get GT image
        GT_path = self.paths_GT[index]
        event_path = self.paths_event[0][index]
        event_LQ = read_mat(event_path)
        ...
        if self.opt['phase'] == 'train':
            ...
            if img_LQ.shape[1] != event_output.shape[1] or img_LQ.shape[0] != event_output.shape[0]:
                event_output = img_LQ[:,:,0:2]

            rnd_h_GT, rnd_w_GT = int(rnd_h * scale), int(rnd_w * scale)
            img_GT = img_GT[rnd_h_GT:rnd_h_GT + GT_size, rnd_w_GT:rnd_w_GT + GT_size, :]

            # augmentation - flip, rotate
            event_output,img_LQ, img_GT = augment([event_output,img_LQ, img_GT], self.opt['use_flip'],
                                          self.opt['use_rot'])
        ...
        event_array = torch.from_numpy(np.ascontiguousarray(np.transpose(event_output, (2, 0, 1)))).float()
        event_LQ = torch.unsqueeze(event_LQ,dim=0)

        ####################################################
        # train_loader要求输入维度必须一致，所以只能先采样，保证维度一致，才能继续进行后续

        if LQ_path is None:
            LQ_path = GT_path

        '''
        img_LQ= torch.Size([3, 128, 128])
        img_GT= torch.Size([3, 256, 256])
        event_array= torch.Size([2, 128, 128])
        event_array_40= torch.Size([40, 128, 128])
        LQ_path= /gdata1/caocz/Event_Camera/SR/EvInt_SR/train/img_lrx4/walking_3_22.jpg
        GT_path= /gdata1/caocz/Event_Camera/SR/EvInt_SR/train/img_lrx2/walking_3_22.jpg
        '''
        # if img_LQ.shape != img_GT.shape:
        #     img_GT = img_LQ

        return {'LQ': img_LQ, 'GT': img_GT, 'Event_array':event_array, 'LQ_path': LQ_path, 'GT_path': GT_path}
