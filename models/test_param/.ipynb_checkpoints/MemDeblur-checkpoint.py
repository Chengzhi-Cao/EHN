...
class ResidualBlocksWithInputConv(nn.Module):
    def __init__(self, in_channels, out_channels=10, num_blocks=30):
        super().__init__()
...
class MemDeblur(nn.Module):
    ...
    def __init__(self, in_nc, out_nc, nf, unf, nb, scale=4):
        ...
        # SCPA
        SCPA_block_f = functools.partial(ResidualBlocksWithInputConv,in_channels=10,out_channels=10)
        self.scale = scale
...
from torchstat import stat
my_model = MemDeblur(in_nc=3,out_nc=3,nf=10,unf=10,nb=16,scale=1)
# 导入模型，输入一张输入图片的尺寸
stat(my_model, (3, 64, 64))